...
async function checkPullRequestsForBranches(context, event, branchName) {
  ...
  const pullRequestsToMerge = allPullRequets.filter(x => !skipPullRequest(context, x));
  
  logger.trace(`Found ${allPullRequets.length} pull requests, ${pullRequestsToMerge.length} to merge.`);
